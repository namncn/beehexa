...
class Widget extends WP_Widget {
	...
	public function get_cached_widget( $args ) {
		...
		$cache = wp_cache_get( apply_filters( 'pdvn_cached_widget_id', $this->widget_id ), 'widget' );
	...
	public function cache_widget( $args, $content ) {
		wp_cache_set( apply_filters( 'pdvn_cached_widget_id', $this->widget_id ), array( $args['widget_id'] => $content ), 'widget' );
	...
	public function flush_widget_cache() {
		wp_cache_delete( apply_filters( 'pdvn_cached_widget_id', $this->widget_id ), 'widget' );
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			 ...
			 */
			$setting[ $key ] = apply_filters( 'pdvn_widget_settings_sanitize_option', $instance[ $key ], $new_instance, $key, $setting );
		}
	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			switch ( $setting['type'] ) {
				...
				default :
					do_action( 'pdvn_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
				break;
			}
		}
	}
